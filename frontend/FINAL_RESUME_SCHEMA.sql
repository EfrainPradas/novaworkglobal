-- ============================================
-- Interview-Magnet Resume System™ - FINAL SETUP
-- Orden correcto: Crear tablas primero, luego agregar FKs
-- ============================================

-- ============================================
-- STEP 1: CREATE BASE TABLES (sin foreign keys problemáticas)
-- ============================================

-- 1. MASTER RESUME TABLE (debe crearse PRIMERO)
...
SELECT '✅ Step 1: New resume builder tables created' as status;

-- ============================================
-- STEP 2: ADD COLUMNS TO EXISTING TABLES
-- ============================================

-- Add columns to certifications (if they don't exist)
DO $$
BEGIN
  -- Add resume_id
  IF NOT EXISTS (
    ...
    ALTER TABLE certifications ADD COLUMN resume_id UUID;
  END IF;
...
-- Add columns to education (if they don't exist)
DO $$
BEGIN
  -- Add resume_id
  IF NOT EXISTS (
    ...
    ALTER TABLE education ADD COLUMN resume_id UUID;
  END IF;
...
SELECT '✅ Step 2: Columns added to existing tables' as status;

-- ============================================
-- STEP 3: ADD FOREIGN KEY CONSTRAINTS (después de que las tablas existen)
-- ============================================

DO $$
BEGIN
  -- Add FK to certifications.resume_id
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'certifications_resume_id_fkey'
  ) THEN
  ...
  -- Add FK to education.resume_id
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'education_resume_id_fkey'
  ) THEN
...
-- ============================================
-- STEP 6: DROP OLD POLICIES (si existen)
-- ============================================
...
-- par_stories policies
CREATE POLICY "Users can view own PAR stories"
  ON par_stories FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own PAR stories"
  ON par_stories FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own PAR stories"
  ON par_stories FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own PAR stories"
  ON par_stories FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own work experience"
  ...
  USING (EXISTS (
    ...
    WHERE user_resumes.id = work_experience.resume_id
...
CREATE POLICY "Users can view own accomplishments"
  ...
  USING (EXISTS (
    ...
    WHERE we.id = accomplishments.work_experience_id
...
-- job_description_analysis policies
CREATE POLICY "Users can view own JD analysis"
  ON job_description_analysis FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own JD analysis"
  ON job_description_analysis FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own JD analysis"
  ON job_description_analysis FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own JD analysis"
  ON job_description_analysis FOR DELETE
  USING (auth.uid() = user_id);
...
SELECT '✅ Step 8: Triggers created' as status;
